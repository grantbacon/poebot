<!DOCTYPE html>
<head>
<title>POE BAWT - about</title>

<link rel="stylesheet" type="text/css" href="/static/css/about.css" />
<link rel="shortcut icon" href="/static/favicon.ico" />
</head>

<body>

<div id="main">
    <h1 id="header">about</h1>
    <hr>
    <span class="about">
        poe bot is a little speech synthesizer that runs on a <a href="http://raspberrypi.org">raspberry pi</a> in my living room attached to a set of speakers.
        <br><br>
        <strong> some technical shit if youre interested </strong><br>
        there are no logs stored in regard to IP address. the IP addresses are stored in temporary memory and are used to assign colors for each unique user.
        one can distinguish (to an extent (colors are recycled after so many unique users)) whether or not a message came from the same person, but in no way can tell who sent the message.
        <br><br>
        checkout this project as it develops <a href="https://github.com/grantbacon/poebot">on github</a>
    </span>
<br><br><br>
    <a href="/">back</a>
</div>




</body>
</html>
